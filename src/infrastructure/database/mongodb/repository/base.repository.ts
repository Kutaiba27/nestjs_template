...
import { IBaseMongoInterface, VDocument } from "../interface";
import { IError } from "@Package/error";
import { QueryOptions } from "mongoose";

export abstract class BaseMongoRepository<V>
  implements IBaseMongoInterface<V>
{
  ...
  async countDocuments({
    ...
    filter?: mongoose.QueryFilter<VDocument<V>>;
    options?: (mongoose.mongo.CountOptions & mongoose.MongooseBaseQueryOptions<VDocument<V>> & mongoose.mongo.Abortable) | null
  }) {
  ...
  async updateOne({
    ...
    filter: mongoose.QueryFilter<VDocument<V>>;
    update: mongoose.UpdateQuery<VDocument<V>>;
    options?: (mongoose.mongo.UpdateOptions & mongoose.MongooseUpdateQueryOptions<VDocument<V>>) | null
    error?: IError;
  }) {
  ...
  async deleteMany({
    ...
    filter?: mongoose.QueryFilter<VDocument<V>>;
    options?: (mongoose.mongo.DeleteOptions & mongoose.MongooseBaseQueryOptions<VDocument<V>>) | null;
  }) {
    return await this.entityModel.deleteMany(filter, options);
  }
}
